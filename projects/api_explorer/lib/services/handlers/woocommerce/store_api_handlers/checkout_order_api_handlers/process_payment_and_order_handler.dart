...
class ProcessPaymentAndOrderOrderHandler implements ApiRequestHandler {
  @override
  Future<Map<String, dynamic>> handleRequest(
    ...
    try {
      ...
      final requestData = ProcessPaymentAndOrderRequestModel(
        ...
        paymentData:
            params['payment_data'] != null ? [params['payment_data']!] : null,
      );

      debugPrint('🔍 Processing payment and order');

      final response =
          await GetIt.I<CheckoutOrderService>().processPaymentAndOrder(
  ...
  Map<String, List<ApiField>> get requiredFields => {
        'POST': [
          ApiField(
            name: 'key',
            label: 'Order Key',
            hint: 'Unique order key for checkout process',
            isRequired: false,
          ...
          ApiField(
            name: 'payment_data',
            label: 'Payment Data',
            hint: 'Additional payment data (JSON string)',
            isRequired: false,
            type: ApiFieldType.text,
          ),
        ],
